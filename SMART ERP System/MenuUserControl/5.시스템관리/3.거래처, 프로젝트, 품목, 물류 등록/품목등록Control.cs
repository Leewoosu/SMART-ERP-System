...
{
    public partial class 품목등록Control : UserControl
    {
        public 품목등록Control()
        {
            InitializeComponent();
        ...
        private void dgv제품_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txb전공정.Text = DB.제품.Search전공정(제품번호);
            txb최종공정.Text = DB.제품.Search최종공정(제품번호);
            txb층.Text = DB.제품.Search층(제품번호);
            txb표준원가.Text = DB.제품.Search표준원가(제품번호);
            txb품목군.Text = DB.제품.Search품목군(제품번호);
            txb후공정.Text = DB.제품.Search후공정(제품번호);
        }
